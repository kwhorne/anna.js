 ...
 * MIT licensed
 *
 * Copyright (C) 2023 Knut W. Horne, https://kwhorne.com
 */
(function( root, factory ) {
	...
	var SLIDES_SELECTOR = '.slides section',
		...
		config = {
			...
			// Disables the default anna.js slide layout so that you can use
			// custom CSS layout
			disableLayout: false,
			...
			// Exposes the anna.js API through window.postMessage
			postMessage: true,

			// Dispatches all anna.js events to the parent window through postMessage
		...
		// Flags if Anna.initialize() has been called
		initialized = false,

		// Flags if anna.js is loaded (has dispatched the 'ready' event)
		loaded = false,
		...
		// A list of registered anna.js plugins
		plugins = {},

		// List of asynchronously loaded anna.js dependencies
		asyncDependencies = [],
	...
	function initialize( options ) {
		...
		dom.wrapper = document.querySelector( '.anna' );
		dom.slides = document.querySelector( '.anna .slides' );

		// Force a layout when the whole page, incl fonts, has loaded
		window.addEventListener( 'load', layout, false );

		var query = Anna.getQueryHash();
	...
	/**
	 * Loads the dependencies of anna.js. Dependencies are
	 * defined via the configuration option 'dependencies'
	 * and will be loaded prior to starting/binding anna.js.
	 * Some dependencies may have an 'async' flag, if so they
	 * will load after anna.js has been started up.
	 */
	function load() {
	...
	/**
	 * Loads all async anna.js dependencies.
	 */
	...
	/**
	 * Starts up anna.js by binding input events and navigating
	...
	function setupDOM() {
		...
		// The right and down arrows in the standard anna.js controls
	...
	function setupPDF() {
		...
		injectStyleSheet( '.anna section>img, .anna section>video, .anna section>iframe{max-width: '+ slideWidth +'px; max-height:'+ slideHeight +'px}' );
	 ...
	 * possible to call all anna.js API methods from another
	 * window. For example:
	 *
	 * annaWindow.postMessage( JSON.stringify({
	 *   method: 'slide',
	 *   args: [ 2 ]
	 * }), '*' );
	 */
		...
		if( config.postMessage ) {
			window.addEventListener( 'message', function ( event ) {
				...
				if( typeof data === 'string' && data.charAt( 0 ) === '{' && data.charAt( data.length - 1 ) === '}' ) {
					...
					if( data.method && typeof Anna[data.method] === 'function' ) {
						Anna[data.method].apply( Anna, data.args );
					}
				}
			}, false );
		}
	...
	function configure( options ) {
		...
		// Abort if anna.js hasn't finished loading, config
	...
	/**
	 * Registers a new plugin with this anna.js instance.
	 *
	 * anna.js waits for all regisered plugins to initialize
	 * before considering itself ready, as long as the plugin
	 * is registered before calling `Anna.initialize()`.
	 */
	function registerPlugin( id, plugin ) {
		...
		if( plugins[id] === undefined ) {
			...
			// If a plugin is registered after anna.js is loaded,
			...
			console.warn( 'anna.js: "'+ id +'" plugin has already been registered' );
		}
	...
	function dispatchEvent( type, args ) {
		...
		// If we're in an iframe, post each anna.js event to the
		// parent window. Used by the notes plugin
		if( config.postMessageEvents && window.parent !== window.self ) {
			window.parent.postMessage( JSON.stringify({ namespace: 'anna', eventName: type, state: getState() }), '*' );
		}
	...
	/**
	 * Hides the mouse pointer when it's on top of the .anna
	 * container.
	 */
	function hideCursor() {
	...
	/**
	 * Updates anna.js to keep in sync with new slide attributes. For
	 * example, if you add a new `data-background-image` you can call
	 * this to have anna.js render the new background image.
	 *
	 ...
	 * after anna.js has already initialized.
	 *
	 * @param {HTMLElement} slide
	...
	function updateBackground( includeAll ) {
		...
		// bubble it to the .anna container
		if( currentSlide ) {
		...
		if( element && element.parentNode ) {
			...
			toArray( element.querySelectorAll( 'video, audio' ) ).forEach( function( el ) {
				if( !el.hasAttribute( 'data-ignore' ) && typeof el.pause === 'function' ) {
					el.setAttribute('data-paused-by-anna', '');
					el.pause();
				}
			} );
		...
		if( !/^[0-9]*$/.test( bits[0] ) && name.length ) {
			...
			if( element ) {
				...
				if ( !isSameNameAsCurrentSlide ) {
					...
					var indices = Anna.getIndices(element);
	 ...
	 *
	 * Fragments will be annaed in the order that they are returned by
	...
	function cueAutoSlide() {
		...
		if( currentSlide && config.autoSlide !== false ) {
			...
			if( autoSlide && !autoSlidePaused && !isPaused() && !isOverview() && ( !Anna.isLastSlide() || availableFragments().next || config.loop === true ) ) {
	...
	function navigateNext() {
		...
		if( nextFragment() === false ) {
			...
			if( routes.down && routes.right && config.loop && Anna.isLastVerticalSlide( currentSlide ) ) {
				routes.down = false;
			}
		...
		if( typeof config.keyboard === 'object' ) {
			...
			for( key in config.keyboard ) {
				...
				if( parseInt( key, 10 ) === keyCode ) {
					...
					// String shortcuts to anna.js API
					else if( typeof value === 'string' && typeof Anna[ value ] === 'function' ) {
						Anna[ value ].call();
					}
		...
		if( triggered === false ) {
			...
			for( key in registeredKeyBindings ) {
				...
				if( parseInt( key, 10 ) === keyCode ) {
					...
					// String shortcuts to anna.js API
					else if( typeof action === 'string' && typeof Anna[ action ] === 'function' ) {
						Anna[ action ].call();
					}
	...
	function onAutoSlidePlayerClick( event ) {
		...
		// Replay
		if( Anna.isLastSlide() && config.loop === false ) {
			slide( 0, 0 );
			resumeAutoSlide();
		}
		// Resume
	...
	Anna = {
		...
		getAnnaElement: function() {
			return dom.wrapper || document.querySelector( '.anna' );
		},
		...
		// Checks if anna.js has been loaded and is ready for use
		isReady: function() {
			return loaded;
		},

		// Forward event binding to the anna DOM element
		addEventListener: function( type, listener, useCapture ) {
			if( 'addEventListener' in window ) {
				Anna.getAnnaElement().addEventListener( type, listener, useCapture );
			}
		},
		removeEventListener: function( type, listener, useCapture ) {
			if( 'addEventListener' in window ) {
				Anna.getAnnaElement().removeEventListener( type, listener, useCapture );
			}
		},
