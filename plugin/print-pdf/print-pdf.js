 ...
 *
 * Example:
 * phantomjs print-pdf.js "http://annajs.com?print-pdf" anna-demo.pdf
 *
...
console.log( 'Export PDF: Reading anna.js config [1/4]' );
	...
	if( config ) {
		...
		printPage.open( inputFile, function( status ) {
			...
			printPage.evaluate( function() {
				Anna.isReady() ? window.callPhantom() : Anna.addEventListener( 'pdf-ready', window.callPhantom );
			} );
		} );
		...
		console.log( 'Export PDF: Unable to read anna.js config. Make sure the input address points to an anna.js page.' );
		phantom.exit( 1 );
