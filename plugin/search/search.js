...
function Hilitor(id, tag)
{
	...
	this.hiliteWords = function(node)
	{
		...
		if(node.nodeType == 3) { // NODE_TEXT
			if((nv = node.nodeValue) && (regs = matchRegex.exec(nv))) {
				...
				var slideIndex = Anna.getIndices(secnode);
	...
	function doSearch() {
		...
		if (matchedSlides) {
			...
			if (matchedSlides.length > currentMatchedIndex) {
				Anna.slide(matchedSlides[currentMatchedIndex].h, matchedSlides[currentMatchedIndex].v);
				currentMatchedIndex++;
			}
		}
	}
	...
	if( window.Anna ) Anna.registerKeyboardShortcut( 'CTRL + Shift + F', 'Search' );
	closeSearch();
